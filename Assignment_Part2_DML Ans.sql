...
Select * from employee
where salary > 3000 and dept_no = 10;
...
add Grade varchar(30);

##  First Method : Using Case But not able to print
Select id, name, marks,
Case
When marks > 80 then 'Distinction'
When marks > 60 then 'First Class'
When marks > 50 then 'Second Class'
When marks > 40 then 'Pass Class'
else 'Fail'
End as grade
From Students
Order by marks desc;
...
Select distinct city from station where right(left(city,1),1) in ('a','e','i','o','u')
Order by city;
...
Select * from employee
where salary > 2000 and hire_date >= date_sub(current_date, interval 10 month)
order by Emp_no Asc;
 
## 7. How much money does the company spend every month on salaries for each department? [table: emp]
